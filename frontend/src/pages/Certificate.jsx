...
function Medal({ tone = "gold", label, size = 86 }) {
  ...
  return (
    <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", margin: "0 4px" }}>
      <svg width={size} height={size * 1.45} viewBox="0 0 100 145" aria-hidden="true">
...
export default function CertificatePage() {
  ...
  const safeFileName = displayName.replace(/\s+/g, "-").replace(/[^a-zA-Z0-9-_]/g, "") || "Student";

  useEffect(() => {
    if (setNameClass) setNameClass(displayName, editClass);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [displayName, editClass]);

  useEffect(() => {
    fireConfetti();
    fireStars();

    if (studentId && !studentId.startsWith("local-") && API) {
  ...
  const downloadPNG = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Computer-Champ-${safeFileName}.png`;
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`Computer-Champ-${safeFileName}.pdf`);
      fireConfetti();
    } finally {
      setDownloading(false);
    }
    ...
    <Layout title="🎉 Congratulations!" testId="certificate-page">
            ...
            <div className="grid grid-cols-2 gap-2">
              {["1", "2"].map((c) => (
                <button
                  ...
                  className={`toy-btn py-3 text-lg ${
                    editClass === c ? "bg-sky-400 text-white" : "bg-white text-sky-600 border-sky-200"
                  }`}
                >
                  Class {c}
                </button>
      ...
      <div className="mt-6 toy-card p-5 text-center bg-gradient-to-r from-amber-50 to-pink-50">
        ...
        <p className="text-slate-700 font-semibold">
          This certificate is <b className="text-emerald-600">100% FREE</b>! Type your child&apos;s name above,
          then download as <b>PDF</b> or <b>Image</b> to print at home.
        </p>
      </div>
    </Layout>
  );
